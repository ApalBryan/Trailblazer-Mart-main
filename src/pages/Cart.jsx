...
function Cart() {
  ...
  const handleOrder = async () => {
    ...
    try {
      // ✅ PRE-CHECK STOCK (handles BOTH stock & pcs)
      for (const item of cartItems) {
        ...
        // 🔥 FIX: support both fields safely
        const currentStock =
          productData.stock ??
          productData.pcs ??
          0;
        ...
        if (item.quantity > currentStock) {
          throw new Error(
            `${productData.brand} only has ${currentStock} stock left`
          );
        }
      }
      ...
      await runTransaction(db, async (transaction) => {
        for (const item of cartItems) {
          ...
          // 🔥 UPDATE BOTH (prevents mismatch issues)
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[999] p-4">
          <div className="bg-white max-w-md w-full rounded-lg shadow-2xl p-6 border-t-8 border-blue-600">
            ...
            <div className="space-y-2 border-y py-4 mb-4 text-sm">
              ...
              <div className="flex justify-between">
                <strong>Date:</strong>
                <span>{slipData.reservationDate}</span>
              </div>
              <div className="flex justify-between">
          ...
          {cartItems.length === 0 ? (
            <div className="cart-empty text-center py-20">
              ...
              <p className="text-gray-500">
                Go back to marketplace to add items.
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="mt-6 flex gap-4">
                ...
                <button
                  onClick={clearCart}
                  className="px-6 py-3 border rounded text-gray-500 font-bold"
                >
                  Clear Cart
                </button>
              </div>
