...
function AdminDashboard() {
  ...
  const toggleStock = async (id, currentStatus) => {
    try {
      ...
      setProducts((prev) =>
        prev.map((p) =>
          p.id === id ? { ...p, inStock: !currentStatus, stock: !currentStatus ? 1 : 0 } : p
        )
      );
    } catch (error) {
      ...
      <div className="mt-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {products.map((product) => (
            <div key={product.id} className="bg-white p-4 rounded-lg shadow flex flex-col gap-3">
              <img src={product.image} alt={product.brand} className="h-32 object-contain mx-auto" />
              ...
              <button
                ...
                className={`mt-2 py-2 rounded-lg text-sm font-semibold transition ${
                  product.inStock ? "bg-red-500 text-white hover:bg-red-600" : "bg-green-500 text-white hover:bg-green-600"
                }`}
              >
