              ...
              <div key={order.id} className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-6 hover:shadow-md transition-shadow">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1">
                      {order.items?.map((item, idx) => (
                        <span key={idx} className="text-xs bg-gray-50 text-gray-600 px-2 py-1 rounded border">
                  ...
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                    Reserved on: {order.createdAt?.toDate() ? order.createdAt.toDate().toLocaleString() : "Unknown"}
                  </p>
                </div>
                  ...
                  <div className="flex gap-2">
                    {isPending && (
                      <>
                        <button
                          ...
                          className="bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-black py-2 px-4 rounded-xl transition shadow-lg shadow-blue-100"
                        >
                          Confirm Payment
                        </button>
                        <button
                          onClick={() => updateOrderStatus(order, "Denied")}
                          className="bg-white border-2 border-red-100 text-red-500 hover:bg-red-50 text-[11px] font-black py-2 px-4 rounded-xl transition"
                        >
                          Deny
                        </button>
                      </>
                    ...
                    {isReserved && (
                      <button
                        ...
                        className="bg-green-600 hover:bg-green-700 text-white text-[11px] font-black py-2 px-6 rounded-xl transition shadow-lg shadow-green-100"
                      >
                        Mark as Claimed
                      </button>
                    )}
